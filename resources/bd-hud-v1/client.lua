...
Citizen.CreateThread(function()
    ...
    local playerDead = IsEntityDead(player) or metadata['inlaststand'] or metadata['isdead']

    -- You can now use `playerDead` as needed
end)

-- Original Code Below:
-- (Partial original code preserved for context and additional manual edits if needed)
...
lastFuelCheck = type(fuel) == 'number' and math.floor(fuel) or 0
    end
    return lastFuelCheck
end

function round(x)
return type(x) == 'number' and (x >= 0 and math.floor(x + 0.5) or math.ceil(x - 0.5)) or 0
end
...
return type(num) == 'number' and math.floor(num + 0.5 * mult) or 0
end
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsPedInAnyVehicle(ped, false) then
            ...
            SendNUIMessage({
                action = "updateFuel",
fuel = type(fuel) == 'number' and math.floor(fuel) or 0
            })
        else
            SendNUIMessage({
                action = "hideFuel"
