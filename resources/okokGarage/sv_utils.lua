...
function giveKeysToPlayer(source, identifier, vehPlate, vehName, isSociety, giveItem) -- Used when first giving keys to a player when they take out their vehicles
	if Config.UseOkokVehicleKeys then
		...
		if Config.KeySystem == "qs-vehiclekeys" then
			TriggerClientEvent(Config.EventPrefix..':giveKeysQS', source, vehPlate, vehName)
		elseif Config.KeySystem == "wasabi-keys" then
			exports.wasabi_carlock:GiveKey(source, vehPlate:match( "^%s*(.-)%s*$" ))
		elseif Config.KeySystem == "qb-vehiclekeys" then
			TriggerClientEvent("vehiclekeys:client:SetOwner", source, vehPlate:match( "^%s*(.-)%s*$" ))
		elseif Config.KeySystem == "jaksam-keys" then
			exports["vehicles_keys"]:giveVehicleKeysToPlayerId(source, vehPlate:match( "^%s*(.-)%s*$" ), vehName)
		else
			-- Add your own key system here
		end
	end
end
...
QBCore.Functions.CreateCallback(Config.EventPrefix..":getPlayerVehicles", function(source, cb, identifier, isSociety, job, isImpound)
	local _source = source
	local xPlayer = QBCore.Functions.GetPlayer(_source)
	local searchPos = 'citizenid'
	...
	local WhereJob = ""
	if isImpound and job ~= nil then
		WhereJob = "OR "..searchPos.." = @job"
	end

	local jsonVehicles = {}
	local allVehicles = {}
	if Config.UseOkokVehicleSales then
		if GetResourceState(Config.okokVehicleSalesName) == "started" then
			local jsonFile = LoadResourceFile(Config.okokVehicleSalesName, "vehicles.json")
			local table = json.decode(jsonFile)
			if table ~= nil then
				for k,v in pairs(table) do
					for k2,v2 in pairs(v) do
						jsonVehicles[k2] = true
					end
				end
			end
		end
	...
	MySQLfetchAll('SELECT *, cast(`state` as signed) as `stored` FROM player_vehicles WHERE '..searchPos..' = @owner '..WhereJob, {
		['@owner'] = identifier,
		['@job'] = job
	}, function(vehiclesDB)
		local time = os.time(os.date("!*t"))
		local category = "car"
		for k,v in ipairs(vehiclesDB) do
			if QBCore.Shared.Vehicles[v.vehicle] == nil then
				local missingVehicles = {}
				print(" ")
				for k,v in ipairs(vehiclesDB) do
					if QBCore.Shared.Vehicles[v.vehicle] == nil and missingVehicles[v.vehicle] == nil then
						missingVehicles[v.vehicle] = true
						print("\27[0;33m Vehicle `"..v.vehicle.."` missing in `shared/vehicles.lua`")
					end
				end
				print(" ")
				return
			end
			v.type = vehicleTypeSet(v.vehicle)
			if type(v.impoundTime) ~= 'number' then
				v.impoundTime = tonumber(v.impoundTime)
				if v.impoundTime == nil then
					v.impoundTime = 0
				end
			end
			if isImpound then
				if Config.ShowVehicleImpoundedWhenExists then
					for k2,v2 in pairs(vehicles) do
						if v.plate == v2.plate and v.stored == 0 then
							if v2.time - time < 0 then
								v.stored = 2
								v.location = Config.Impound[1].name
								v.impoundTime = time
							end
						elseif v.plate == v2.plate and v.stored == 1 then
							vehicles[k2] = nil
						end
					end
				end
			...
			if not jsonVehicles[v.plate] then
	...
	MySQLfetchAll('SELECT *, cast(`state` as signed) as `stored` FROM player_vehicles WHERE plate = @plate', {
		...
		if db ~= nil then
			...
			if db.type == garage_type then
				...
				if xPlayer.PlayerData.citizenid == db.citizenid or sharedWith ~= nil and sharedWith[xPlayer.PlayerData.citizenid] or xPlayer.PlayerData.job.name:lower() == db.citizenid:lower() or not Config.StoreOwnedVehiclesOnly then
			...
			if Config.SocietyVehiclesList[xPlayer.PlayerData.job.name:lower()] ~= nil then
				for k, v in pairs(Config.SocietyVehiclesList[xPlayer.PlayerData.job.name:lower()]) do
					if activeKeys[xPlayer.PlayerData.citizenid] ~= nil then
						...
						if v.plate:match( "^%s*(.-)%s*$" ) == vehicle_plate:match( "^%s*(.-)%s*$" ) or Config.RandomPlateSociety then
							storeVehicle(vehicle_plate, vehicle_props, tyreCondition, doorCondition, windowCondition, _source, netID, garage_id, garageInfo)
							stored = true
							break
						end
			...
			if not stored and not Config.StoreOwnedVehiclesOnly then
				storeVehicle(vehicle_plate, vehicle_props, tyreCondition, doorCondition, windowCondition, _source, netID, garage_id, garageInfo)
				stored = true
			end
		end
...
RegisterNetEvent(Config.EventPrefix..":checkIfTimeIsFinished", function(plate, index, vehicle_name, spawnLocationID, job)
	...
	MySQLfetchAll('SELECT *, cast(`state` as signed) as `stored` FROM player_vehicles WHERE citizenid = @owner AND plate = @plate OR citizenid = @job AND plate = @plate', {
		...
		if vehicle[1] ~= nil then
			...
			if type(db.impoundTime) ~= 'number' then
				db.impoundTime = tonumber(db.impoundTime)
				if db.impoundTime == nil then
					db.impoundTime = 0
				end
			end
			if Config.ShowVehicleImpoundedWhenExists then
						...
						if db ~= nil then
							if db.stored == 0 then
								...
								MySQLexecute('UPDATE player_vehicles SET `state` = @state, `location` = @location, `impoundTime` = @impoundTime WHERE plate = @plate', {
									...
									['@impoundTime'] = time + (Config.DefaultImpoundTime or 0),
								}, function (rowsChanged)
									if rowsChanged > 0 then
										vehicles[k] = nil
									end
